...
extraction_data <- read_csv("resources/library-preps/02_allPlants_Toronto_extractionData_allQC.csv")

# Load file with Toronto plant phenotype data
tor_allPlants <- read_csv("resources/reference/allPlants_Toronto.csv") %>% 
  select(Population, Plant, HCN_Result, Locus.Li, Locus.Ac)

# Load file with Habitat data
habitat <- read_csv("resources/library-preps/01_torontoPops_toExtract.csv") %>% 
...
write_csv(allData, path = "resources/library-preps/03_allPlants_Toronto_allData.csv")
